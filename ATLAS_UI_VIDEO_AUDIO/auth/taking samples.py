...
detector = cv2.CascadeClassifier('ATLAS_UI_VIDEO_AUDIO\\auth\\haarcascade_frontalface_default.xml')
...
while True:
    ...
    for (x,y,w,h) in faces:
        ...
        cv2.imwrite("ATLAS_UI_VIDEO_AUDIO\\auth\\samples\\face." + str(face_id) + '.' + str(count) + ".jpg", converted_image[y:y+h,x:x+w])
        # To capture & Save images into the datasets folder

        cv2.imshow('image1', img) #Used to display an image in a window
