...
def find_contact_to_video_call(query):
            ...
            for row in reader:
                if len(row) > max(name_index, mobile_index):
                    ...
                    if original_name.lower() == search_lower:
                        ...
                        start_whatsapp_video_call(row[mobile_index].strip())
                        return f"Starting video call to {original_name}. Please wait" 
...
def start_whatsapp_video_call(phone_number):
    whatsapp_uri = f"whatsapp://send?phone={phone_number}"
    os.startfile(whatsapp_uri)
    time.sleep(5)
    for i in range(10): # press 9 tab to navaigate the video call button
        p.press('tab')
        time.sleep(0.1)
    p.sleep(0.2)
    p.press('enter')
